...
public class MainActivity extends Activity {
    ...
    void addSidebarSection(String gid,GroupModel group){
        if(group!=null){
            ...
            TextView n=text("▾ "+group.name,compact?12:14,true);TextView sum=text(fmt(groupTotal(gid)),compact?11:13,true);sum.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);gh.addView(n,w(0,dp(40),1));gh.addView(sum,w(0,dp(40),0.9f));gh.setTag("GROUP:"+gid);
    ...
    View sidebarItem(TableModel t,String gid){
        ...
        TextView title=text(t.title,compact?14:16,true);title.setTextColor(t.id.equals(selectedId)?Color.WHITE:navy);TextView meta=text(timeText(t.updated)+" · "+t.dataRowCount()+" dòng",compact?10:12,false);meta.setTextColor(t.id.equals(selectedId)?Color.rgb(220,232,245):Color.rgb(90,112,138));item.addView(title);item.addView(meta);
    ...
    void renderCalcGrid(TableModel t){
        ...
        ensureBlankCalc(t);int shown=Math.max(8,t.calcRows.size());for(int i=0;i<shown;i++){final int row=i;CalcRow r=i<t.calcRows.size()?t.calcRows.get(i):null;LinearLayout rr=gridRow();TextView st=cell(String.valueOf(i+1),14,false,Gravity.CENTER);st.setOnClickListener(v->{activeRow=row;renderGrid();});rr.addView(st,w(0,dp(compact?50:56),0.45f));TextView p=cell(r==null||r.price==0?"":plain(r.price),16,false,Gravity.END|Gravity.CENTER_VERTICAL);TextView q=cell(r==null||r.qty==0?"":plain(r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);TextView total=cell(r==null||r.price==0||r.qty==0?"":fmt(r.price*r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);if(row==activeRow&&"price".equals(activeField))markActive(p);if(row==activeRow&&"qty".equals(activeField))markActive(q);p.setOnClickListener(v->{activeRow=row;activeField="price";ensureRow(t,row);renderGrid();});q.setOnClickListener(v->{activeRow=row;activeField="qty";ensureRow(t,row);renderGrid();});rr.addView(p,w(0,dp(compact?50:56),2));rr.addView(q,w(0,dp(compact?50:56),1));rr.addView(total,w(0,dp(compact?50:56),2));body.addView(rr);}
    }
    ...
    void renderCancelGrid(TableModel t){
        ...
        ScrollView sv=new ScrollView(this);LinearLayout body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);sv.addView(body);gridHost.addView(sv,new LinearLayout.LayoutParams(-1,0,1));ensureBlankCancel(t);int shown=Math.max(8,t.cancelRows.size());for(int i=0;i<shown;i++){final int row=i;CancelRow r=i<t.cancelRows.size()?t.cancelRows.get(i):null;LinearLayout rr=gridRow();rr.addView(cell(String.valueOf(i+1),14,false,Gravity.CENTER),w(0,dp(compact?50:56),0.45f));TextView a=cell(r==null?"":r.agent,16,false,Gravity.START|Gravity.CENTER_VERTICAL);TextView q=cell(r==null||r.qty==0?"":String.valueOf(r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);if(row==activeRow)markActive(q);a.setOnClickListener(v->{ensureCancelRow(t,row);editAgent(t,row);});q.setOnClickListener(v->{activeRow=row;activeField="qty";ensureCancelRow(t,row);renderGrid();});rr.addView(a,w(0,dp(56),3));rr.addView(q,w(0,dp(compact?50:56),2));body.addView(rr);}
    }
