...
public class MainActivity extends Activity {
    ...
    void buildScreen(){
        ...
        compact=getResources().getConfiguration().screenWidthDp<500;
        ...
        if(compact){
            ...
            tr1.addView(tableBtn,w(0,dp(58),1));tr1.addView(del,w(0,dp(58),1));tr1.addView(undoBtn,w(0,dp(58),1));
            tr2.addView(share,w(0,dp(58),1));tr2.addView(quick1000,w(0,dp(58),1));tr2.addView(add,w(0,dp(58),1));
            top.addView(tr1,new LinearLayout.LayoutParams(-1,dp(61)));
            top.addView(tr2,new LinearLayout.LayoutParams(-1,dp(61)));
        }else{
        ...
        int sideDp=Math.max(82,Math.min(225,(int)(swDp*0.22f)));
    ...
    void addSidebarSection(String gid,GroupModel group){
        if(group!=null){
            ...
            TextView n=text("▾ "+group.name,getResources().getConfiguration().screenWidthDp<500?12:14,true);TextView sum=text(fmt(groupTotal(gid)),getResources().getConfiguration().screenWidthDp<500?11:13,true);sum.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);gh.addView(n,w(0,dp(40),1));gh.addView(sum,w(0,dp(40),0.9f));gh.setTag("GROUP:"+gid);
    ...
    View sidebarItem(TableModel t,String gid){
        ...
        TextView title=text(t.title,getResources().getConfiguration().screenWidthDp<500?14:16,true);title.setTextColor(t.id.equals(selectedId)?Color.WHITE:navy);TextView meta=text(timeText(t.updated)+" · "+t.dataRowCount()+" dòng",getResources().getConfiguration().screenWidthDp<500?10:12,false);meta.setTextColor(t.id.equals(selectedId)?Color.rgb(220,232,245):Color.rgb(90,112,138));item.addView(title);item.addView(meta);
    ...
    void renderCalcGrid(TableModel t){
        LinearLayout head=gridRow();head.addView(cell("STT",14,false,Gravity.CENTER),w(0,dp(58),0.45f));head.addView(cell("Đơn giá",14,false,Gravity.CENTER),w(0,dp(58),2));head.addView(cell("SL",14,false,Gravity.CENTER),w(0,dp(58),1));head.addView(cell("Thành tiền",14,false,Gravity.END|Gravity.CENTER_VERTICAL),w(0,dp(58),2));gridHost.addView(head);
        ...
        ensureBlankCalc(t);int shown=Math.max(8,t.calcRows.size());for(int i=0;i<shown;i++){final int row=i;CalcRow r=i<t.calcRows.size()?t.calcRows.get(i):null;LinearLayout rr=gridRow();TextView st=cell(String.valueOf(i+1),14,false,Gravity.CENTER);st.setOnClickListener(v->{activeRow=row;renderGrid();});rr.addView(st,w(0,dp(56),0.45f));TextView p=cell(r==null||r.price==0?"":plain(r.price),16,false,Gravity.END|Gravity.CENTER_VERTICAL);TextView q=cell(r==null||r.qty==0?"":plain(r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);TextView total=cell(r==null||r.price==0||r.qty==0?"":fmt(r.price*r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);if(row==activeRow&&"price".equals(activeField))markActive(p);if(row==activeRow&&"qty".equals(activeField))markActive(q);p.setOnClickListener(v->{activeRow=row;activeField="price";ensureRow(t,row);renderGrid();});q.setOnClickListener(v->{activeRow=row;activeField="qty";ensureRow(t,row);renderGrid();});rr.addView(p,w(0,dp(56),2));rr.addView(q,w(0,dp(56),1));rr.addView(total,w(0,dp(56),2));body.addView(rr);}
    }

    void renderCancelGrid(TableModel t){
        LinearLayout head=gridRow();head.addView(cell("STT",14,false,Gravity.CENTER),w(0,dp(58),0.45f));head.addView(cell("Tên đại lý",14,false,Gravity.START|Gravity.CENTER_VERTICAL),w(0,dp(58),3));head.addView(cell("Số lượng",14,false,Gravity.END|Gravity.CENTER_VERTICAL),w(0,dp(58),2));gridHost.addView(head);
        ScrollView sv=new ScrollView(this);LinearLayout body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);sv.addView(body);gridHost.addView(sv,new LinearLayout.LayoutParams(-1,0,1));ensureBlankCancel(t);int shown=Math.max(8,t.cancelRows.size());for(int i=0;i<shown;i++){final int row=i;CancelRow r=i<t.cancelRows.size()?t.cancelRows.get(i):null;LinearLayout rr=gridRow();rr.addView(cell(String.valueOf(i+1),14,false,Gravity.CENTER),w(0,dp(56),0.45f));TextView a=cell(r==null?"":r.agent,16,false,Gravity.START|Gravity.CENTER_VERTICAL);TextView q=cell(r==null||r.qty==0?"":String.valueOf(r.qty),16,false,Gravity.END|Gravity.CENTER_VERTICAL);if(row==activeRow)markActive(q);a.setOnClickListener(v->{ensureCancelRow(t,row);editAgent(t,row);});q.setOnClickListener(v->{activeRow=row;activeField="qty";ensureCancelRow(t,row);renderGrid();});rr.addView(a,w(0,dp(56),3));rr.addView(q,w(0,dp(56),2));body.addView(rr);}
    }
    ...
    void showTablePicker(View anchor){
        PopupMenu p=new PopupMenu(this,anchor);
        for(int i=0;i<tables.size();i++){
            TableModel t=tables.get(i);
            String label=(t.id.equals(selectedId)?"✓ ":"")+t.title;
            p.getMenu().add(0,1000+i,i,label);
        }
        android.view.SubMenu manage=p.getMenu().addSubMenu("Quản lý bảng");
        manage.add("Tạo nhóm");
        manage.add("Đổi tên bảng hiện tại");
        manage.add("Chuyển bảng vào nhóm");
        p.setOnMenuItemClickListener(i->{
            int id=i.getItemId();
            if(id>=1000&&id<1000+tables.size()){
                selectedId=tables.get(id-1000).id;activeRow=0;activeField="cancel".equals(selected().type)?"qty":"price";renderAll();return true;
            }
            String s=i.getTitle().toString();
            if(s.startsWith("Tạo"))createGroupDialog();
            else if(s.startsWith("Đổi"))renameCurrent();
            else if(s.startsWith("Chuyển"))moveCurrentGroup();
            return true;
        });
        p.show();
    }
