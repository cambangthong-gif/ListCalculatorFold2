...
public class MainActivity extends Activity {
    ...
    final ArrayList<TotalGroupModel> totalGroups=new ArrayList<>();
    LinearLayout sidebar, gridHost, keypadHost;
    ScrollView gridScroll;
    FrameLayout coverFrame;
    View coverDrawerView, coverScrimView, coverEdgeHandle;
    boolean coverDrawerOpen=false;
    float coverDrawerTouchX=0, coverDrawerTouchY=0;
    TextView sidebarGrandView;
    ...
    final ArrayDeque<String> undoStack=new ArrayDeque<>(), undoLabelStack=new ArrayDeque<>();
    final ArrayDeque<String> redoStack=new ArrayDeque<>(), redoLabelStack=new ArrayDeque<>();
    static final int HISTORY_LIMIT=20;
    ItemTouchHelper rowTouchHelper=null;
    final ArrayList<Long> cashHistory=new ArrayList<>();
    ...
    final WeakHashMap<View,Runnable> pendingDeleteHoldCancels=new WeakHashMap<>();
    ...
    @Override public void onCreate(Bundle b){
        ...
        String ch=p.getString(CASH_HISTORY_KEY,"");
        if(ch!=null&&!ch.isEmpty()){
            for(String s:ch.split("\\|")){
                try{long v=Long.parseLong(s);if(v>0)cashHistory.add(v);}catch(Exception ignored){}
            }
        }
        load();
    ...
    @Override public void onBackPressed(){
        if(coverDrawerOpen){closeCoverDrawer();return;}
        super.onBackPressed();
    }
    ...
    @Override public boolean dispatchTouchEvent(MotionEvent e){
        ...
        if(action==MotionEvent.ACTION_DOWN){
            ...
            if(!globalDragging && Math.abs(dx)>dp(10) && Math.abs(dx)>Math.abs(dy)*1.25f){
                globalDragging=true;
                // Child cell may stop receiving MOVE after dispatchTouchEvent takes
                // ownership of the swipe, so cancel all pending delete-holds here.
                cancelAllPendingDeleteHolds();
            }
            ...
            if(wasDragging)cancelAllPendingDeleteHolds();
    ...
    void openCoverDrawer(){
        if(!compact||coverDrawerView==null)return;
        coverDrawerOpen=true;
        if(coverScrimView!=null){
            coverScrimView.setVisibility(View.VISIBLE);
            coverScrimView.animate().alpha(.28f).setDuration(160).start();
        }
        coverDrawerView.setVisibility(View.VISIBLE);
        coverDrawerView.animate().translationX(0f).setDuration(190).setInterpolator(new DecelerateInterpolator()).start();
        haptic(coverDrawerView);
    }

    void closeCoverDrawer(){
        if(coverDrawerView==null)return;
        coverDrawerOpen=false;
        float w=coverDrawerView.getWidth()>0?coverDrawerView.getWidth():dp(320);
        coverDrawerView.animate().translationX(-w).setDuration(170).setInterpolator(new AccelerateInterpolator()).withEndAction(()->{
            if(!coverDrawerOpen)coverDrawerView.setVisibility(View.INVISIBLE);
        }).start();
        if(coverScrimView!=null)coverScrimView.animate().alpha(0f).setDuration(150).withEndAction(()->{
            if(!coverDrawerOpen)coverScrimView.setVisibility(View.GONE);
        }).start();
    }

    void installCoverDrawerSwipe(View edge,View drawer){
        if(edge!=null)edge.setOnTouchListener((v,e)->{
            if(e.getAction()==MotionEvent.ACTION_DOWN){coverDrawerTouchX=e.getRawX();coverDrawerTouchY=e.getRawY();return true;}
            if(e.getAction()==MotionEvent.ACTION_UP){
                float dx=e.getRawX()-coverDrawerTouchX,dy=e.getRawY()-coverDrawerTouchY;
                if(dx>dp(34)&&Math.abs(dx)>Math.abs(dy)*1.25f)openCoverDrawer();
                return true;
            }
            return true;
        });
        if(drawer!=null)drawer.setOnTouchListener((v,e)->{
            if(e.getAction()==MotionEvent.ACTION_DOWN){coverDrawerTouchX=e.getRawX();coverDrawerTouchY=e.getRawY();return false;}
            if(e.getAction()==MotionEvent.ACTION_UP){
                float dx=e.getRawX()-coverDrawerTouchX,dy=e.getRawY()-coverDrawerTouchY;
                if(dx<-dp(52)&&Math.abs(dx)>Math.abs(dy)*1.2f){closeCoverDrawer();return true;}
            }
            return false;
        });
    }

    View buildSidebarBottomPanel(){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(8),dp(8),dp(8),dp(8));

        LinearLayout totalCard=new LinearLayout(this);totalCard.setGravity(Gravity.CENTER_VERTICAL);totalCard.setPadding(dp(12),dp(6),dp(10),dp(6));
        GradientDrawable bg=new GradientDrawable();bg.setColor(Color.WHITE);bg.setStroke(dp(1),Color.rgb(225,228,232));bg.setCornerRadius(dp(18));totalCard.setBackground(bg);
        TextView chart=text("▥",24,true);chart.setTextColor(accent);chart.setGravity(Gravity.CENTER);
        LinearLayout labels=new LinearLayout(this);labels.setOrientation(LinearLayout.VERTICAL);
        TextView cap=text("Tổng cộng",11,false);cap.setTextColor(muted);
        sidebarGrandView=text("0",17,true);sidebarGrandView.setTextColor(ink);
        labels.addView(cap);labels.addView(sidebarGrandView);
        TextView arrow=text("›",26,true);arrow.setGravity(Gravity.CENTER);arrow.setTextColor(navy);
        totalCard.addView(chart,new LinearLayout.LayoutParams(dp(42),dp(48)));
        totalCard.addView(labels,new LinearLayout.LayoutParams(0,dp(48),1));
        totalCard.addView(arrow,new LinearLayout.LayoutParams(dp(30),dp(48)));
        totalCard.setOnClickListener(v->{haptic(v);showTotalComposer();});
        totalCard.setOnLongClickListener(v->{haptic(v);showDashboard();return true;});
        box.addView(totalCard,new LinearLayout.LayoutParams(-1,dp(62)));

        if(!totalGroups.isEmpty()){
            Button savedTotals=smallActionButton("∑ Nhóm cộng tổng  ("+totalGroups.size()+")");
            savedTotals.setTextColor(accent);
            savedTotals.setOnClickListener(v->{haptic(v);showTotalComposer();});
            box.addView(savedTotals,new LinearLayout.LayoutParams(-1,dp(42)));
        }
    ...
    void buildScreen(){
        ...
        tableBtn.setOnClickListener(v->{haptic(v);openTableNavigation();});
        del.setOnClickListener(v->{haptic(v);showMultiDeleteDialog();});
        undoBtn.setOnClickListener(v->{haptic(v);undoDelete();});
        undoBtn.setOnLongClickListener(v->{haptic(v);redoLast();return true;});
        undoBtn.setContentDescription("Hoàn tác; nhấn giữ để làm lại");
        ...
        if(compact){
            ...
            labels.setOnClickListener(v->{haptic(v);openCoverDrawer();});
        ...
        ScrollView leftScroll=new ScrollView(this);leftScroll.setFillViewport(true);
        sidebar=new LinearLayout(this);sidebar.setOrientation(LinearLayout.VERTICAL);sidebar.setPadding(dp(8),dp(8),dp(8),dp(8));sidebar.setBackgroundColor(Color.rgb(248,250,253));
        leftScroll.addView(sidebar);installScrollHaptics(leftScroll);
        int sideDp=responsiveSideDp(swDp);

        LinearLayout sideColumn=new LinearLayout(this);sideColumn.setOrientation(LinearLayout.VERTICAL);sideColumn.setBackgroundColor(Color.rgb(248,250,253));
        if(compact){
            View drawerTopSpacer=new View(this);drawerTopSpacer.setBackgroundColor(Color.rgb(248,250,253));sideColumn.addView(drawerTopSpacer,new LinearLayout.LayoutParams(-1,0));applyTopSystemInset(drawerTopSpacer);
        }
        sideColumn.addView(leftScroll,new LinearLayout.LayoutParams(-1,0,1));
        sideColumn.addView(buildSidebarBottomPanel(),new LinearLayout.LayoutParams(-1,dp(122)));
        if(!compact)middle.addView(sideColumn,new LinearLayout.LayoutParams(dp(sideDp),-1));
        ...
        currentGroupTotal.setTextColor(Color.rgb(55,72,98));
        ...
        if(compact){
            coverFrame=new FrameLayout(this);
            coverFrame.setBackgroundColor(Color.rgb(247,249,252));
            coverFrame.addView(root,new FrameLayout.LayoutParams(-1,-1));

            coverScrimView=new View(this);coverScrimView.setBackgroundColor(Color.BLACK);coverScrimView.setAlpha(0f);coverScrimView.setVisibility(View.GONE);
            coverScrimView.setOnClickListener(v->closeCoverDrawer());
            coverFrame.addView(coverScrimView,new FrameLayout.LayoutParams(-1,-1));

            int drawerDp=Math.min(340,Math.max(292,(int)(swDp*.86f)));
            coverDrawerView=sideColumn;
            GradientDrawable drawerBg=new GradientDrawable();drawerBg.setColor(Color.rgb(248,250,253));drawerBg.setCornerRadii(new float[]{0,0,dp(24),dp(24),dp(24),dp(24),0,0});
            coverDrawerView.setBackground(drawerBg);coverDrawerView.setElevation(dp(12));coverDrawerView.setVisibility(View.INVISIBLE);
            FrameLayout.LayoutParams drawerLp=new FrameLayout.LayoutParams(dp(drawerDp),-1,Gravity.START);
            coverFrame.addView(coverDrawerView,drawerLp);

            coverEdgeHandle=new View(this);coverEdgeHandle.setBackgroundColor(Color.TRANSPARENT);
            FrameLayout.LayoutParams edgeLp=new FrameLayout.LayoutParams(dp(24),-1,Gravity.START);
            coverFrame.addView(coverEdgeHandle,edgeLp);
            installCoverDrawerSwipe(coverEdgeHandle,coverDrawerView);
            coverDrawerView.post(()->coverDrawerView.setTranslationX(-coverDrawerView.getWidth()));
            setContentView(coverFrame);
        }else{
            coverFrame=null;coverDrawerView=null;coverScrimView=null;coverEdgeHandle=null;coverDrawerOpen=false;
            setContentView(root);
        }
        applyRootBottomSafeArea(root);

        addCalc.setOnClickListener(v->{haptic(v);addCalcTable(true);});
        addCancel.setOnClickListener(v->{haptic(v);addCancelTable(true);}); del.setOnClickListener(v->{haptic(v);showMultiDeleteDialog();}); undoBtn.setOnClickListener(v->{haptic(v);undoDelete();}); share.setOnClickListener(v->{haptic(v);showShareChooser();}); quick1000.setOnClickListener(v->{haptic(v);cycleNumberFormat();}); tableBtn.setOnClickListener(v->{haptic(v);openTableNavigation();});
        renderAll();
    }
    ...
    GroupModel findGroup(String gid){
        if(gid==null||gid.isEmpty())return null;
        for(GroupModel g:groups)if(gid.equals(g.id))return g;
        return null;
    }

    boolean isEffectivelyLocked(TableModel t){
        if(t==null)return false;
        GroupModel g=findGroup(t.groupId);
        return t.locked || (g!=null&&g.locked);
    }

    void toggleGroupLock(GroupModel g){
        if(g==null)return;
        pushUndo(g.locked?"Mở khóa nhóm":"Khóa nhóm");
        g.locked=!g.locked;
        saveNow();renderAll();
        Toast.makeText(this,g.locked?"Đã khóa nhóm và các bảng":"Đã mở khóa nhóm",Toast.LENGTH_SHORT).show();
    }
    ...
    void addSidebarModeButton(){
        ...
        Button add=smallActionButton("+");add.setTextSize(24);add.setTextColor(Color.WHITE);add.setContentDescription("Tạo bảng hoặc nhóm");
        GradientDrawable addBg=new GradientDrawable();addBg.setColor(Color.rgb(66,133,244));addBg.setCornerRadius(dp(16));add.setBackground(addBg);
        search.setOnClickListener(v->{haptic(v);if(compact)closeCoverDrawer();showTableManagerSheet();});
        add.setOnClickListener(v->{haptic(v);showAddMenu(v);});

        bar.addView(label,new LinearLayout.LayoutParams(0,dp(44),1));
        if(!sidebarCompactMode){
            bar.addView(search,new LinearLayout.LayoutParams(dp(44),dp(40)));
            bar.addView(add,new LinearLayout.LayoutParams(dp(44),dp(40)));
        }else{
            Button wide=smallActionButton("›");wide.setTextColor(accent);wide.setOnClickListener(v->{haptic(v);toggleSidebarMode();});
            bar.addView(wide,new LinearLayout.LayoutParams(dp(44),dp(40)));
        }
        sidebar.addView(bar,new LinearLayout.LayoutParams(-1,dp(54)));
    }
    ...
    void renderSidebar(){
        ...
        for(GroupModel g:orderedGroups())if(!g.pinned)addSidebarSection(g.id,g);
        if(sidebarGrandView!=null)sidebarGrandView.setText(fmt(allTablesTotal()));
    ...
    View sidebarItem(TableModel t,String gid){
        ...
        View.OnClickListener select=v->{selectedId=t.id;activeRow=0;activeField="cancel".equals(t.type)?"qty":"price";explicitCellSelection=false;if(compact)closeCoverDrawer();renderAll();};
    ...
    void attachNativeRowReorder(TableModel t,RecyclerView.Adapter adapter){
        rowTouchHelper=new ItemTouchHelper(new ItemTouchHelper.Callback(){
            ...
            @Override public int getMovementFlags(RecyclerView rv,RecyclerView.ViewHolder vh){
                ...
                if(p<0||p>last||isEffectivelyLocked(t))return makeMovementFlags(0,0);
                return makeMovementFlags(ItemTouchHelper.UP|ItemTouchHelper.DOWN,0);
            }
            @Override public boolean onMove(RecyclerView rv,RecyclerView.ViewHolder from,RecyclerView.ViewHolder target){
                int a=from.getBindingAdapterPosition(),b=target.getBindingAdapterPosition();
                int last=lastDataRowIndex(t);
                if(a<0||b<0||a>last||b>last)return false;
                if(!moving){pushUndo("Kéo dòng");moving=true;}
                if("cancel".equals(t.type))Collections.swap(t.cancelRows,a,b);
            ...
            @Override public void clearView(RecyclerView rv,RecyclerView.ViewHolder vh){
                ...
                if(moving){
                    moving=false;t.updated=System.currentTimeMillis();save();
                    updateLiveTotals(t);renderSidebar();
                }
            }
        });
        rowTouchHelper.attachToRecyclerView(gridRecycler);
    }
    ...
    void renderCalcGrid(TableModel t){
        ...
        CalcAdapter calcAdapter=new CalcAdapter(t);
        gridRecycler.setAdapter(calcAdapter);
        attachNativeRowReorder(t,calcAdapter);
    ...
    void renderCancelGrid(TableModel t){
        ...
        gridRecycler.setItemAnimator(null);
        gridRecycler.setOverScrollMode(View.OVER_SCROLL_IF_CONTENT_SCROLLS);
        CancelAdapter cancelAdapter=new CancelAdapter(t);
        gridRecycler.setAdapter(cancelAdapter);
        attachNativeRowReorder(t,cancelAdapter);
        gridHost.addView(gridRecycler,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1));
    }
    ...
    void cancelAllPendingDeleteHolds(){
        if(pendingDeleteHoldCancels.isEmpty())return;
        ArrayList<Runnable> cancels=new ArrayList<>(pendingDeleteHoldCancels.values());
        pendingDeleteHoldCancels.clear();
        for(Runnable r:cancels)if(r!=null)r.run();
    }
    ...
    void installDeleteRowHold(View v,Runnable action){
        ...
        final Runnable[] fireRef={null};

        final Runnable cancel=()->{
            armed[0]=false;
            if(fireRef[0]!=null)v.removeCallbacks(fireRef[0]);
            pendingDeleteHoldCancels.remove(v);
        };

        final Runnable fire=()->{
            // Tuyệt đối không mở Xóa dòng nếu gesture ngang toàn màn hình
            // đã được nhận diện là đổi bảng.
            if(!armed[0] || globalDragging){
                cancel.run();
                return;
            }
            armed[0]=false;
        ...
        v.setOnTouchListener((view,e)->{
            ...
            if(a==MotionEvent.ACTION_DOWN){
                ...
                downY[0]=e.getY();
                armed[0]=true;
                pendingDeleteHoldCancels.put(view,cancel);
                view.postDelayed(fire,deleteRowHoldMs);

            }else if(a==MotionEvent.ACTION_MOVE){
                float dx=Math.abs(e.getX()-downX[0]);
                float dy=Math.abs(e.getY()-downY[0]);

                // Gesture ngang cần hủy giữ-xóa rất sớm. Điều này tránh trường hợp
                // dispatchTouchEvent bắt swipe ngang và child không nhận MOVE tiếp theo.
                float horizontalCancel=Math.max(dp(5),slop*0.65f);
                float normalCancel=slop*1.6f;

                if(globalDragging ||
                        (dx>horizontalCancel && dx>dy*1.05f) ||
                        dx>normalCancel || dy>normalCancel){
                    cancel.run();
                }
    ...
    class CalcAdapter extends RecyclerView.Adapter<CalcAdapter.H>{
        ...
        @Override public void onBindViewHolder(H h,int row){
            ...
            h.st.setTextColor(r!=null&&!r.blank()?accent:ink);
            ...
            if(row==activeRow)rowBg=Color.rgb(240,246,255);
            ...
            h.st.setContentDescription(r!=null&&!r.blank()?"Giữ và kéo để di chuyển dòng "+(row+1):"Dòng "+(row+1));
    ...
    class CancelAdapter extends RecyclerView.Adapter<CancelAdapter.H>{
        ...
        @Override public void onBindViewHolder(H h,int row){
            ...
            h.st.setText((r!=null&&!r.blank()?"☰ ":"")+String.valueOf(row+1));
            h.st.setTextColor(r!=null&&!r.blank()?Color.rgb(194,65,12):ink);
            ...
            int rowBg=(row==activeRow)?Color.rgb(255,248,238):(row%2==0?Color.WHITE:Color.rgb(250,252,255));
    ...
    void updateLiveTotals(TableModel t){
        ...
        // Tổng cộng ở sidebar/drawer phải cập nhật ngay theo từng phím bấm.
        if(sidebarGrandView!=null)sidebarGrandView.setText(fmt(allTablesTotal()));
    ...
    void rememberCashBase(double value){
        long v=(long)Math.round(value);if(v<=0)return;
        cashHistory.remove(v);cashHistory.add(0,v);
        while(cashHistory.size()>5)cashHistory.remove(cashHistory.size()-1);
        StringBuilder s=new StringBuilder();
        for(long x:cashHistory){if(s.length()>0)s.append("|");s.append(x);}
        getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(CASH_HISTORY_KEY,s.toString()).apply();
    }
    ...
    void showCashRemainderDialog(){
        ...
        LinearLayout quickCash=new LinearLayout(this);
        Button useTableTotal=smallActionButton("+ Tổng bảng");
        Button useGroupTotal=smallActionButton("+ Tổng nhóm");
        Button historyCash=smallActionButton("Gần đây");
        useGroupTotal.setEnabled(t.groupId!=null&&!t.groupId.isEmpty());
        quickCash.addView(useTableTotal,new LinearLayout.LayoutParams(0,dp(38),1));
        quickCash.addView(useGroupTotal,new LinearLayout.LayoutParams(0,dp(38),1));
        quickCash.addView(historyCash,new LinearLayout.LayoutParams(0,dp(38),1));
        baseCard.addView(quickCash);
        ...
        useTableTotal.setOnClickListener(v->{
            haptic(v);double cur=parseNum(amount.getText().toString().replace(".","").replace(",",""));
            amount.setText(String.valueOf((long)Math.round(cur+t.total())));amount.setSelection(amount.length());
        });
        useGroupTotal.setOnClickListener(v->{
            haptic(v);double cur=parseNum(amount.getText().toString().replace(".","").replace(",",""));
            double add=(t.groupId==null||t.groupId.isEmpty())?0:groupTotal(t.groupId);
            amount.setText(String.valueOf((long)Math.round(cur+add)));amount.setSelection(amount.length());
        });
        historyCash.setOnClickListener(v->{
            haptic(v);
            if(cashHistory.isEmpty()){Toast.makeText(this,"Chưa có lịch sử",Toast.LENGTH_SHORT).show();return;}
            String[] opts=new String[cashHistory.size()];
            for(int i=0;i<cashHistory.size();i++)opts[i]=fmt(cashHistory.get(i));
            new AlertDialog.Builder(this).setTitle("Tổng tiền gần đây").setItems(opts,(d,i)->{
                amount.setText(String.valueOf(cashHistory.get(i)));amount.setSelection(amount.length());
            }).show();
        });
    ...
    void handleKey(String field,String key){
        TableModel t=selected();if(t==null)return;if(isEffectivelyLocked(t)){Toast.makeText(this,"Bảng/nhóm đang khóa",Toast.LENGTH_SHORT).show();return;}
    ...
    String stateJson(){
        try{
            ...
            JSONArray tga=new JSONArray();for(TotalGroupModel g:totalGroups)tga.put(g.json());
            root.put("groups",ga).put("tables",ta).put("totalGroups",tga);
    ...
    void pushUndo(String label){
        String snapshot=stateJson();
        undoSnapshot=snapshot;
        undoLabel=label==null?"Thao tác":label;
        undoStack.addLast(snapshot);
        undoLabelStack.addLast(undoLabel);
        while(undoStack.size()>HISTORY_LIMIT){undoStack.removeFirst();if(!undoLabelStack.isEmpty())undoLabelStack.removeFirst();}
        redoStack.clear();redoLabelStack.clear();
    ...
    void undoDelete(){
        if(!undoStack.isEmpty()){
            String current=stateJson();
            String snapshot=undoStack.removeLast();
            String label=undoLabelStack.isEmpty()?"Thao tác":undoLabelStack.removeLast();
            redoStack.addLast(current);redoLabelStack.addLast(label);
        ...
        if(undoSnapshot!=null){
            String s=undoSnapshot,label=undoLabel;undoSnapshot=null;undoLabel="";
            restoreStateJson(s);Toast.makeText(this,"Đã hoàn tác: "+label,Toast.LENGTH_SHORT).show();return;
        }
        if(lastDeleted==null)return;
        tables.add(Math.max(0,Math.min(lastDeletedIndex,tables.size())),lastDeleted);
        selectedId=lastDeleted.id;lastDeleted=null;lastDeletedIndex=-1;save();renderAll();
    }

    void redoLast(){
        if(redoStack.isEmpty()){
            Toast.makeText(this,"Không có thao tác để làm lại",Toast.LENGTH_SHORT).show();return;
        }
        String current=stateJson();
        String snapshot=redoStack.removeLast();
    ...
    void restoreStateJson(String json){
        try{
            ...
            groups.clear();tables.clear();totalGroups.clear();collapsedGroups.clear();
            ...
            JSONArray tga=o.optJSONArray("totalGroups");if(tga!=null)for(int i=0;i<tga.length();i++)totalGroups.add(TotalGroupModel.from(tga.getJSONObject(i)));
    ...
    void showAddMenu(View anchor){
        ...
        p.getMenu().add("Bảng hủy vé");p.getMenu().add("Tạo từ mẫu");p.getMenu().add("Lưu bảng hiện tại thành mẫu");
        p.setOnMenuItemClickListener(i->{
            String s=i.getTitle().toString();
            if(s.startsWith("Bảng hủy"))addCancelTable(true);
            else if(s.startsWith("Bảng tính"))addCalcTable(true);
            else if(s.startsWith("Tạo từ mẫu"))createFromTemplate();
            else if(s.startsWith("Lưu bảng"))saveCurrentAsTemplate();
            return true;
        });
        p.show();
    }
    ...
    void showTableManagerSheet(String preselectId){
        ...
        final int[] managerFilter={0}; // 0=tất cả,1=bảng,2=nhóm,3=chưa nhóm
        LinearLayout filterBar=new LinearLayout(this);
        Button fAll=smallActionButton("Tất cả");
        Button fTables=smallActionButton("Bảng");
        Button fGroups=smallActionButton("Nhóm");
        Button fUngrouped=smallActionButton("Chưa nhóm");
        filterBar.addView(fAll,new LinearLayout.LayoutParams(0,dp(40),1));
        filterBar.addView(fTables,new LinearLayout.LayoutParams(0,dp(40),1));
        filterBar.addView(fGroups,new LinearLayout.LayoutParams(0,dp(40),1));
        filterBar.addView(fUngrouped,new LinearLayout.LayoutParams(0,dp(40),1));
        root.addView(filterBar,new LinearLayout.LayoutParams(-1,dp(42)));
        ...
        Button settings=smallActionButton("Cài đặt");
        Button dashboard=smallActionButton("Tổng quan");
        ...
        tools.addView(dashboard,new LinearLayout.LayoutParams(0,dp(managerScreenW>=600?46:44),1));
        root.addView(tools);
        ...
        Button selAll=smallActionButton("Chọn hết");
        ...
        selectionBar.addView(selAll,new LinearLayout.LayoutParams(dp(78),dp(40)));
        ...
        Button selectAll=smallActionButton("Chọn tất cả");
        ...
        rebuild[0]=()->{
            ...
            selAll.setText(groupMode?"Chọn hết nhóm":"Chọn hết bảng");
            ...
            if(managerFilter[0]!=3)for(GroupModel g:orderedGroups()){
                ...
                TextView gname=text((g.pinned?"📌 ":"")+(g.locked?"🔒 ":"")+(collapsedGroups.contains(g.id)?"▸ ":"▾ ")+g.name,16,true);
                ...
                if(managerFilter[0]!=2 && !collapsedGroups.contains(g.id)){
        ...
        View.OnClickListener filterClick=v->{
            haptic(v);
            if(v==fAll)managerFilter[0]=0;
            else if(v==fTables)managerFilter[0]=1;
            else if(v==fGroups)managerFilter[0]=2;
            else managerFilter[0]=3;
            rebuild[0].run();
        };
        fAll.setOnClickListener(filterClick);fTables.setOnClickListener(filterClick);
        fGroups.setOnClickListener(filterClick);fUngrouped.setOnClickListener(filterClick);
        ...
        settings.setOnClickListener(v->{haptic(v);showSettingsDialog();});
        dashboard.setOnClickListener(v->{haptic(v);showDashboard();});
        ...
        selAll.setOnClickListener(v->{
            haptic(v);
            if(!selectedGroupIds.isEmpty()){
                if(selectedGroupIds.size()==groups.size())selectedGroupIds.clear();
                else{
                    selectedIds.clear();
                    selectedGroupIds.clear();
                    for(GroupModel g:groups)selectedGroupIds.add(g.id);
                }
            }else{
                if(selectedIds.size()==tables.size())selectedIds.clear();
                    ...
                    selectedGroupIds.clear();
                    selectedIds.clear();
                    for(TableModel tb:tables)selectedIds.add(tb.id);
                }
            }
            rebuild[0].run();
        });
        ...
        selectAll.setOnClickListener(v->{
            haptic(v);
            String[] opts={"Chọn tất cả bảng","Chọn tất cả nhóm","Bỏ chọn tất cả"};
            new AlertDialog.Builder(this)
    ...
    boolean tableMatches(TableModel t,String q){
        ...
        String gn=groupNameFor(t.groupId);
        if(gn!=null&&gn.toLowerCase(Locale.getDefault()).contains(q))return true;
        if(fmt(t.total()).toLowerCase(Locale.getDefault()).contains(q))return true;
        if(managerTime(t.created).toLowerCase(Locale.getDefault()).contains(q)||managerTime(t.updated).toLowerCase(Locale.getDefault()).contains(q))return true;
    ...
    ArrayList<TableModel> managerTablesForGroup(String gid,String q){
        ...
        else if(managerSortMode==4)c=(x,y)->Long.compare(y.created,x.created);
        else if(managerSortMode==5)c=(x,y)->Double.compare(y.total(),x.total());
        else if(managerSortMode==6)c=(x,y)->Integer.compare(y.dataRowCount(),x.dataRowCount());
        if(c!=null)Collections.sort(a,c);
        return a;
    }

    void showSortDialog(Runnable after){
        String[] opts={"Thứ tự kéo tay","Tên A → Z","Sửa mới nhất","Sửa cũ nhất","Tạo mới nhất","Tổng lớn → nhỏ","Nhiều dòng → ít"};
    ...
    View managerTableRow(TableModel t,String gid,HashSet<String> selectedIds,Dialog dlg,Runnable rebuild){
        ...
        TextView meta=text((isEffectivelyLocked(t)?"🔒 • ":"")+t.dataRowCount()+" dòng • "+fmt(t.total()),12,false);meta.setTextColor(muted);
        ...
        row.setOnTouchListener((v,e)->{
            ...
            if(e.getActionMasked()==MotionEvent.ACTION_UP){
                ...
                if(dx<0 && Math.abs(dx)>width*.58f && Math.abs(dx)>Math.abs(dy)*.85f){
                    haptic(v);
                    if(isEffectivelyLocked(t)){Toast.makeText(this,"Bảng/nhóm đang khóa",Toast.LENGTH_SHORT).show();return true;}
    ...
    void showManagerTableActions(TableModel t,Dialog manager,Runnable rebuild){
        ...
        new AlertDialog.Builder(this).setTitle(t.title).setItems(actions,(d,i)->{
            ...
            else if(i==5){if(isEffectivelyLocked(t)){Toast.makeText(this,"Bảng/nhóm đang khóa",Toast.LENGTH_SHORT).show();return;}new AlertDialog.Builder(this).setTitle("Xóa "+t.title+"?")
    ...
    void showManagerGroupActions(GroupModel g,Dialog manager,Runnable rebuild){
        String[] actions={"Thu gọn / Mở rộng",g.pinned?"Bỏ ghim nhóm":"Ghim nhóm",g.locked?"Mở khóa nhóm":"Khóa nhóm","Di chuyển nhóm lên","Di chuyển nhóm xuống","Đổi tên nhóm","Copy cả nhóm","Chia sẻ nhóm","Xóa toàn bộ số lượng trong nhóm","Xóa nhóm và toàn bộ bảng"};
        new AlertDialog.Builder(this).setTitle(g.name).setItems(actions,(d,i)->{
            ...
            else if(i==2){toggleGroupLock(g);rebuild.run();}
            else if(i==3){moveGroupBy(g,-1);rebuild.run();renderSidebar();}
            else if(i==4){moveGroupBy(g,1);rebuild.run();renderSidebar();}
            else if(i==5){manager.dismiss();renameGroup(g);}
            else if(i==6){copyGroup(g);rebuild.run();}
            else if(i==7){showGroupExportChooser(g);}
            else if(i==8)confirmClearGroupQuantities(g,()->{rebuild.run();renderAll();});
            else{
                manager.dismiss();
                deleteGroup(g);
            }
    ...
    void showMoveSelectedDialog(HashSet<String> selectedIds,Dialog manager,Runnable rebuild){
        ...
        new AlertDialog.Builder(this).setTitle("Chuyển "+selectedIds.size()+" bảng").setItems(names.toArray(new String[0]),(d,i)->{
            ...
            for(TableModel t:new ArrayList<>(tables))if(selectedIds.contains(t.id)){tables.remove(t);t.groupId=gid;t.updated=System.currentTimeMillis();moving.add(t);}
    ...
    void quickDeleteTable(TableModel t){
        if(t==null)return;
        if(isEffectivelyLocked(t)){Toast.makeText(this,"Bảng/nhóm đang khóa",Toast.LENGTH_SHORT).show();renderAll();return;}
        pushUndo("Xóa nhanh bảng");
    ...
    void showGroupMenu(View anchor,GroupModel g){
        ...
        p.getMenu().add(g.locked?"Mở khóa nhóm":"Khóa nhóm");
        ...
        p.setOnMenuItemClickListener(i->{
            ...
            if(s.startsWith("Mở")||s.startsWith("Thu")){
                ...
                togglePinGroup(g);
            }else if(s.contains("khóa nhóm")||s.contains("Khóa nhóm")){
                toggleGroupLock(g);
    ...
    void clearQuantitiesInGroup(GroupModel g){
        if(g==null)return;
        if(g.locked){
            Toast.makeText(this,"Nhóm đang khóa",Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    void confirmClearGroupQuantities(GroupModel g,Runnable after){
        if(g==null)return;
        if(g.locked){Toast.makeText(this,"Nhóm đang khóa",Toast.LENGTH_SHORT).show();return;}
        new AlertDialog.Builder(this)
    ...
    JSONArray loadTemplates(){
        try{
            String s=getSharedPreferences(PREFS,MODE_PRIVATE).getString(TEMPLATE_KEY,"[]");
            return new JSONArray(s==null?"[]":s);
        }catch(Exception e){return new JSONArray();}
    }

    void saveCurrentAsTemplate(){
        TableModel src=selected();if(src==null)return;
        EditText e=new EditText(this);e.setHint("Tên mẫu");e.setText(src.title);e.setSingleLine();
        new AlertDialog.Builder(this).setTitle("Lưu bảng thành mẫu").setView(padded(e))
            .setPositiveButton("Lưu",(d,w)->{
                String name=e.getText().toString().trim();if(name.isEmpty())return;
                try{
                    JSONObject o=src.json();o.put("templateName",name);
                    JSONArray a=loadTemplates();a.put(o);
                    getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(TEMPLATE_KEY,a.toString()).apply();
                    Toast.makeText(this,"Đã lưu mẫu "+name,Toast.LENGTH_SHORT).show();
                }catch(Exception ex){Toast.makeText(this,"Không lưu được mẫu",Toast.LENGTH_SHORT).show();}
            }).setNegativeButton("Hủy",null).show();
    }

    void createFromTemplate(){
        JSONArray a=loadTemplates();
        if(a.length()==0){Toast.makeText(this,"Chưa có mẫu. Mở một bảng rồi chọn Lưu thành mẫu.",Toast.LENGTH_LONG).show();return;}
        String[] names=new String[a.length()];
        for(int i=0;i<a.length();i++)names[i]=a.optJSONObject(i).optString("templateName","Mẫu "+(i+1));
        new AlertDialog.Builder(this).setTitle("Tạo bảng từ mẫu").setItems(names,(d,i)->{
            try{
                TableModel t=TableModel.from(a.getJSONObject(i));
                t.id=id();t.title=names[i];t.created=System.currentTimeMillis();t.updated=t.created;t.groupId="";
                tables.add(t);selectedId=t.id;saveNow();renderAll();
            }catch(Exception ex){Toast.makeText(this,"Không tạo được từ mẫu",Toast.LENGTH_SHORT).show();}
        }).setNeutralButton("Xóa mẫu",(d,w)->showDeleteTemplateDialog()).show();
    }

    void showDeleteTemplateDialog(){
        JSONArray a=loadTemplates();if(a.length()==0)return;
        String[] names=new String[a.length()];boolean[] checked=new boolean[a.length()];
        for(int i=0;i<a.length();i++)names[i]=a.optJSONObject(i).optString("templateName","Mẫu "+(i+1));
        new AlertDialog.Builder(this).setTitle("Xóa mẫu").setMultiChoiceItems(names,checked,(d,i,on)->checked[i]=on)
            .setPositiveButton("Xóa",(d,w)->{
                JSONArray out=new JSONArray();
                for(int i=0;i<a.length();i++)if(!checked[i])out.put(a.optJSONObject(i));
                getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(TEMPLATE_KEY,out.toString()).apply();
            }).setNegativeButton("Hủy",null).show();
    }
    ...
    void editAgent(TableModel t,int row){if(isEffectivelyLocked(t)){Toast.makeText(this,"Bảng/nhóm đang khóa",Toast.LENGTH_SHORT).show();return;}activeRow=row;CancelRow r=t.cancelRows.get(row);EditText e=new EditText(this);e.setHint("Tên đại lý");e.setText(r.agent);e.setSingleLine();AlertDialog dlg=new AlertDialog.Builder(this).setTitle("Tên đại lý").setView(padded(e)).setPositiveButton("Lưu",(d,w)->{r.agent=e.getText().toString().trim();t.updated=System.currentTimeMillis();ensureBlankCancel(t);save();renderAll();}).setNegativeButton("Hủy",null).create();dlg.setOnShowListener(x->{e.requestFocus();dlg.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);});dlg.show();}
    ...
    double selectedCustomTotal(Set<String> tableIds,Set<String> groupIds){
        LinkedHashSet<String> unique=new LinkedHashSet<>();
        if(tableIds!=null)unique.addAll(tableIds);
        if(groupIds!=null)for(String gid:groupIds){
            for(TableModel t:tables)if(gid!=null&&gid.equals(t.groupId))unique.add(t.id);
        }
        double total=0;
        for(String id:unique){
            TableModel t=findTable(id);
            if(t!=null)total+=t.total();
        }
        return total;
    }

    int selectedCustomTableCount(Set<String> tableIds,Set<String> groupIds){
        LinkedHashSet<String> unique=new LinkedHashSet<>();
        if(tableIds!=null)unique.addAll(tableIds);
        if(groupIds!=null)for(String gid:groupIds){
            for(TableModel t:tables)if(gid!=null&&gid.equals(t.groupId))unique.add(t.id);
        }
        int n=0;for(String id:unique)if(findTable(id)!=null)n++;
        return n;
    }

    void showTotalGroupActions(TotalGroupModel tg,Runnable refresh,Set<String> selectedTables,Set<String> selectedGroups){
        String[] opts={"Dùng nhóm cộng tổng này","Đổi tên","Xóa nhóm cộng tổng"};
        new AlertDialog.Builder(this).setTitle(tg.name).setItems(opts,(d,i)->{
            if(i==0){
                selectedTables.clear();selectedTables.addAll(tg.tableIds);
                selectedGroups.clear();selectedGroups.addAll(tg.groupIds);
                if(refresh!=null)refresh.run();
            }else if(i==1){
                EditText e=new EditText(this);e.setSingleLine();e.setText(tg.name);e.setSelection(e.length());
                new AlertDialog.Builder(this).setTitle("Đổi tên nhóm cộng tổng").setView(padded(e))
                    .setPositiveButton("Lưu",(x,w)->{
                        String s=e.getText().toString().trim();
                        if(!s.isEmpty()){tg.name=s;saveNow();if(refresh!=null)refresh.run();}
                    }).setNegativeButton("Hủy",null).show();
            }else{
                new AlertDialog.Builder(this).setTitle("Xóa nhóm cộng tổng?")
                    .setMessage(tg.name+"\n\nChỉ xóa nhóm cộng tổng đã lưu, không xóa bảng hoặc nhóm dữ liệu.")
                    .setPositiveButton("Xóa",(x,w)->{
                        totalGroups.remove(tg);saveNow();renderAll();
                        if(refresh!=null)refresh.run();
                    }).setNegativeButton("Hủy",null).show();
            }
        }).show();
    ...
    void showTotalComposer(){
        final Dialog dlg=new Dialog(this);
        dlg.requestWindowFeature(Window.FEATURE_NO_TITLE);

        final HashSet<String> selectedTables=new HashSet<>();
        final HashSet<String> selectedGroups=new HashSet<>();

        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);
        root.setPadding(dp(compact?10:16),dp(10),dp(compact?10:16),dp(12));
        root.setBackgroundColor(Color.rgb(248,250,253));

        LinearLayout header=new LinearLayout(this);header.setGravity(Gravity.CENTER_VERTICAL);
        LinearLayout titleBox=new LinearLayout(this);titleBox.setOrientation(LinearLayout.VERTICAL);
        TextView title=text("Cộng tổng bảng & nhóm",compact?18:21,true);
        TextView sub=text("Chọn tự do bảng và nhóm • không cộng trùng bảng",11,false);sub.setTextColor(muted);
        ...
        LinearLayout totalCard=new LinearLayout(this);totalCard.setGravity(Gravity.CENTER_VERTICAL);
        totalCard.setPadding(dp(14),dp(8),dp(14),dp(8));
        GradientDrawable tbg=new GradientDrawable();tbg.setColor(Color.rgb(232,240,254));tbg.setCornerRadius(dp(20));totalCard.setBackground(tbg);
        LinearLayout totalLabels=new LinearLayout(this);totalLabels.setOrientation(LinearLayout.VERTICAL);
        TextView totalCap=text("Tổng đã chọn",12,false);totalCap.setTextColor(muted);
        TextView totalCount=text("0 bảng",11,false);totalCount.setTextColor(muted);
        totalLabels.addView(totalCap);totalLabels.addView(totalCount);
        TextView totalValue=text("0",compact?24:28,true);totalValue.setTextColor(accent);totalValue.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);
        totalCard.addView(totalLabels,new LinearLayout.LayoutParams(0,dp(58),1));
        totalCard.addView(totalValue,new LinearLayout.LayoutParams(0,dp(58),1.2f));
        root.addView(totalCard,new LinearLayout.LayoutParams(-1,dp(72)));

        LinearLayout quick=new LinearLayout(this);quick.setPadding(0,dp(6),0,dp(4));
        Button allTables=smallActionButton("Tất cả bảng");
        Button allGroups=smallActionButton("Tất cả nhóm");
        Button clear=smallActionButton("Bỏ chọn");
        quick.addView(allTables,new LinearLayout.LayoutParams(0,dp(42),1));
        quick.addView(allGroups,new LinearLayout.LayoutParams(0,dp(42),1));
        quick.addView(clear,new LinearLayout.LayoutParams(0,dp(42),1));
        root.addView(quick);

        ScrollView scroll=new ScrollView(this);installScrollHaptics(scroll);
        LinearLayout list=new LinearLayout(this);list.setOrientation(LinearLayout.VERTICAL);list.setPadding(0,dp(2),0,dp(6));
        scroll.addView(list);
        root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));

        LinearLayout bottom=new LinearLayout(this);bottom.setPadding(0,dp(6),0,0);
        Button saveGroup=smallActionButton("+ Tạo nhóm cộng tổng");
        saveGroup.setTextColor(accent);
        Button dashboard=smallActionButton("Tổng quan");
        bottom.addView(saveGroup,new LinearLayout.LayoutParams(0,dp(50),1.5f));
        bottom.addView(dashboard,new LinearLayout.LayoutParams(0,dp(50),1));
        root.addView(bottom);

        final Runnable[] refresh={null};
        refresh[0]=()->{
            totalValue.setText(fmt(selectedCustomTotal(selectedTables,selectedGroups)));
            totalCount.setText(selectedCustomTableCount(selectedTables,selectedGroups)+" bảng thực tế");
            list.removeAllViews();

            if(!totalGroups.isEmpty()){
                TextView savedHead=text("NHÓM CỘNG TỔNG ĐÃ LƯU",11,true);savedHead.setTextColor(muted);savedHead.setPadding(dp(6),dp(8),0,dp(4));
                list.addView(savedHead,new LinearLayout.LayoutParams(-1,dp(34)));
                for(TotalGroupModel tg:new ArrayList<>(totalGroups)){
                    LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(10),dp(4),dp(6),dp(4));
                    GradientDrawable bg=new GradientDrawable();bg.setColor(Color.WHITE);bg.setStroke(dp(1),Color.rgb(225,228,232));bg.setCornerRadius(dp(16));row.setBackground(bg);
                    LinearLayout info=new LinearLayout(this);info.setOrientation(LinearLayout.VERTICAL);
                    TextView n=text("∑  "+tg.name,14,true);
                    TextView m=text(selectedCustomTableCount(tg.tableIds,tg.groupIds)+" bảng • "+fmt(selectedCustomTotal(tg.tableIds,tg.groupIds)),11,false);m.setTextColor(muted);
                    info.addView(n);info.addView(m);
                    Button use=smallActionButton("Dùng");
                    TextView more=text("⋮",24,true);more.setGravity(Gravity.CENTER);more.setTextColor(muted);
                    row.addView(info,new LinearLayout.LayoutParams(0,dp(56),1));
                    row.addView(use,new LinearLayout.LayoutParams(dp(62),dp(40)));
                    row.addView(more,new LinearLayout.LayoutParams(dp(42),dp(48)));
                    use.setOnClickListener(v->{
                        haptic(v);selectedTables.clear();selectedTables.addAll(tg.tableIds);
                        selectedGroups.clear();selectedGroups.addAll(tg.groupIds);refresh[0].run();
                    });
                    more.setOnClickListener(v->{haptic(v);showTotalGroupActions(tg,refresh[0],selectedTables,selectedGroups);});
                    list.addView(row,new LinearLayout.LayoutParams(-1,dp(62)));
                }
            }

            TextView gh=text("NHÓM",11,true);gh.setTextColor(muted);gh.setPadding(dp(6),dp(10),0,dp(4));
            list.addView(gh,new LinearLayout.LayoutParams(-1,dp(38)));
            for(GroupModel g:orderedGroups()){
                LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(8),dp(2),dp(6),dp(2));
                CheckBox cb=new CheckBox(this);cb.setButtonTintList(android.content.res.ColorStateList.valueOf(accent));cb.setChecked(selectedGroups.contains(g.id));
                LinearLayout info=new LinearLayout(this);info.setOrientation(LinearLayout.VERTICAL);
                TextView n=text(g.name,14,true);
                TextView m=text(inGroup(g.id).size()+" bảng • "+fmt(groupTotal(g.id)),11,false);m.setTextColor(muted);
                info.addView(n);info.addView(m);
                row.addView(cb,new LinearLayout.LayoutParams(dp(48),dp(56)));
                row.addView(info,new LinearLayout.LayoutParams(0,dp(56),1));
                cb.setOnCheckedChangeListener((b,on)->{
                    if(on)selectedGroups.add(g.id);else selectedGroups.remove(g.id);
                    totalValue.setText(fmt(selectedCustomTotal(selectedTables,selectedGroups)));
                    totalCount.setText(selectedCustomTableCount(selectedTables,selectedGroups)+" bảng thực tế");
                });
                info.setOnClickListener(v->{cb.setChecked(!cb.isChecked());});
                list.addView(row,new LinearLayout.LayoutParams(-1,dp(58)));
            }

            TextView th=text("BẢNG",11,true);th.setTextColor(muted);th.setPadding(dp(6),dp(10),0,dp(4));
            list.addView(th,new LinearLayout.LayoutParams(-1,dp(38)));
            for(TableModel t:tables){
                LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(8),dp(2),dp(6),dp(2));
                ...
                cb.setOnCheckedChangeListener((b,on)->{
                    if(on)selectedTables.add(t.id);else selectedTables.remove(t.id);
                    totalValue.setText(fmt(selectedCustomTotal(selectedTables,selectedGroups)));
                    totalCount.setText(selectedCustomTableCount(selectedTables,selectedGroups)+" bảng thực tế");
                });
                info.setOnClickListener(v->{cb.setChecked(!cb.isChecked());});
        ...
        allTables.setOnClickListener(v->{
            haptic(v);selectedGroups.clear();selectedTables.clear();
            for(TableModel t:tables)selectedTables.add(t.id);refresh[0].run();
        });
        allGroups.setOnClickListener(v->{
            haptic(v);selectedTables.clear();selectedGroups.clear();
            for(GroupModel g:groups)selectedGroups.add(g.id);refresh[0].run();
        });
        clear.setOnClickListener(v->{haptic(v);selectedTables.clear();selectedGroups.clear();refresh[0].run();});
        close.setOnClickListener(v->dlg.dismiss());
        dashboard.setOnClickListener(v->{haptic(v);showDashboard();});

        saveGroup.setOnClickListener(v->{
            haptic(v);
            if(selectedCustomTableCount(selectedTables,selectedGroups)==0){
                Toast.makeText(this,"Chưa chọn bảng hoặc nhóm",Toast.LENGTH_SHORT).show();return;
            }
            EditText e=new EditText(this);e.setSingleLine();e.setHint("Ví dụ: Doanh thu khu vực Bắc");
            new AlertDialog.Builder(this).setTitle("Tạo nhóm cộng tổng").setView(padded(e))
                .setMessage("Nhóm cộng tổng chỉ lưu cách chọn để tính tổng. Không di chuyển bảng và không thay đổi nhóm dữ liệu hiện có.")
                .setPositiveButton("Tạo",(d,w)->{
                    String name=e.getText().toString().trim();
                    if(name.isEmpty())name="Nhóm cộng tổng "+(totalGroups.size()+1);
                    TotalGroupModel tg=new TotalGroupModel();tg.id=id();tg.name=name;
                    tg.tableIds.addAll(selectedTables);tg.groupIds.addAll(selectedGroups);
                    totalGroups.add(tg);saveNow();refresh[0].run();
                    Toast.makeText(this,"Đã tạo nhóm cộng tổng",Toast.LENGTH_SHORT).show();
                }).setNegativeButton("Hủy",null).show();
        });

        dlg.setContentView(root);
        dlg.setOnShowListener(x->{
            Window w=dlg.getWindow();if(w==null)return;
            w.setBackgroundDrawableResource(android.R.color.transparent);
            int sw=getResources().getConfiguration().screenWidthDp;
            if(sw<600){
                w.setGravity(Gravity.BOTTOM);
                w.setLayout(WindowManager.LayoutParams.MATCH_PARENT,(int)(getResources().getDisplayMetrics().heightPixels*.90f));
            }else{
                w.setGravity(Gravity.CENTER);
                w.setLayout(Math.min(getResources().getDisplayMetrics().widthPixels-dp(40),dp(760)),
                        (int)(getResources().getDisplayMetrics().heightPixels*.84f));
            }
        });
        dlg.show();
    }
    ...
    void showDashboard(){
        int calc=0,cancel=0,locked=0;
        double total=0;
        TableModel newest=null;
        for(TableModel t:tables){
            if("cancel".equals(t.type))cancel++;else calc++;
            if(isEffectivelyLocked(t))locked++;
            total+=t.total();
            if(newest==null||t.updated>newest.updated)newest=t;
        }

        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(18),dp(10),dp(18),dp(8));
        TextView big=text(fmt(total),26,true);big.setTextColor(accent);big.setGravity(Gravity.CENTER);
        box.addView(text("TỔNG TOÀN BỘ",12,true));box.addView(big,new LinearLayout.LayoutParams(-1,dp(52)));
        box.addView(text(groups.size()+" nhóm • "+tables.size()+" bảng • "+calc+" bảng tính • "+cancel+" hủy vé",14,false));
        box.addView(text(locked+" bảng đang khóa/thuộc nhóm khóa",12,false));

        if(newest!=null){
            TextView recent=text("Sửa gần nhất: "+newest.title+" • "+managerTime(newest.updated),12,false);
            recent.setTextColor(muted);box.addView(recent);
        }

        box.addView(new Space(this),new LinearLayout.LayoutParams(1,dp(10)));
        for(GroupModel g:orderedGroups()){
            TextView r=text((g.locked?"🔒 ":"")+g.name+"   "+fmt(groupTotal(g.id))+"   • "+inGroup(g.id).size()+" bảng",13,true);
            r.setPadding(dp(8),dp(7),dp(8),dp(7));r.setTextColor(g.locked?red:ink);
            r.setOnClickListener(v->showGroupQuickView(g));
            box.addView(r);
        }

        new AlertDialog.Builder(this).setTitle("Tổng quan").setView(box).setPositiveButton("Đóng",null).show();
    }

    void showGroupQuickView(GroupModel g){
        if(g==null)return;
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(16),dp(8),dp(16),dp(8));
        TextView total=text("Tổng nhóm  "+fmt(groupTotal(g.id)),22,true);total.setTextColor(accent);box.addView(total);
        TextView meta=text(inGroup(g.id).size()+" bảng • "+groupManagerDates(g),11,false);meta.setTextColor(muted);box.addView(meta);
        for(TableModel t:inGroup(g.id)){
            TextView row=text(t.title+"   "+fmt(t.total()),13,true);
            row.setPadding(dp(6),dp(7),dp(6),dp(7));
            row.setOnClickListener(v->{selectedId=t.id;activeRow=0;renderAll();});
            box.addView(row);
        }
        new AlertDialog.Builder(this).setTitle((g.locked?"🔒 ":"")+g.name).setView(box)
            .setNeutralButton("Chia sẻ",(d,w)->shareGroup(g))
    ...
    void showSettingsDialog(){
        ...
        installScrollHaptics(settingsScroll);

        LinearLayout box=new LinearLayout(this);
        box.setOrientation(LinearLayout.VERTICAL);
        box.setPadding(dp(18),dp(10),dp(18),dp(18));
        settingsScroll.addView(box,new ScrollView.LayoutParams(
        ...
        TextView holdTitle=text("Thời gian nhấn giữ để xóa dòng",14,true);
        holdTitle.setPadding(0,dp(16),0,0);
        box.addView(holdTitle);

        TextView holdDesc=text("Áp dụng giống nhau ở màn ngoài và màn trong. Giữ ô Đơn giá / SL / Thành tiền / Tên đại lý / Số lượng để mở Xóa dòng. Ô STT có ký hiệu ☰ vẫn dành riêng cho kéo dòng.",11,false);
        holdDesc.setTextColor(muted);
        box.addView(holdDesc);

        LinearLayout holdCard=new LinearLayout(this);
        holdCard.setOrientation(LinearLayout.VERTICAL);
        holdCard.setPadding(dp(12),dp(10),dp(12),dp(10));
        GradientDrawable holdCardBg=new GradientDrawable();
        holdCardBg.setColor(Color.rgb(243,246,250));
        holdCardBg.setStroke(dp(1),Color.rgb(218,220,224));
        holdCardBg.setCornerRadius(dp(18));
        holdCard.setBackground(holdCardBg);

        TextView holdValue=text("",16,true);
        holdValue.setTextColor(accent);
        holdValue.setGravity(Gravity.CENTER);
        holdCard.addView(holdValue,new LinearLayout.LayoutParams(-1,dp(34)));

        SeekBar deleteHoldSeek=new SeekBar(this);
        deleteHoldSeek.setMax(18); // 300ms + 18*50ms = 1200ms
        int initialHoldProgress=Math.max(0,Math.min(18,(deleteRowHoldMs-300)/50));
        deleteHoldSeek.setProgress(initialHoldProgress);
        holdCard.addView(deleteHoldSeek,new LinearLayout.LayoutParams(-1,dp(50)));

        LinearLayout holdEnds=new LinearLayout(this);holdEnds.setGravity(Gravity.CENTER_VERTICAL);
        TextView fast=text("0,3 giây • nhanh",10,false);fast.setTextColor(muted);
        TextView slow=text("1,2 giây • chậm",10,false);slow.setTextColor(muted);slow.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);
        holdEnds.addView(fast,new LinearLayout.LayoutParams(0,dp(22),1));
        holdEnds.addView(slow,new LinearLayout.LayoutParams(0,dp(22),1));
        holdCard.addView(holdEnds);

        LinearLayout holdPresets=new LinearLayout(this);
        holdPresets.setPadding(0,dp(6),0,0);
        Button hold300=smallActionButton("0,3s");
        Button hold500=smallActionButton("0,5s");
        Button hold800=smallActionButton("0,8s");
        Button hold1200=smallActionButton("1,2s");
        holdPresets.addView(hold300,new LinearLayout.LayoutParams(0,dp(40),1));
        holdPresets.addView(hold500,new LinearLayout.LayoutParams(0,dp(40),1));
        holdPresets.addView(hold800,new LinearLayout.LayoutParams(0,dp(40),1));
        holdPresets.addView(hold1200,new LinearLayout.LayoutParams(0,dp(40),1));
        holdCard.addView(holdPresets);

        Button testDeleteHold=smallActionButton("Giữ nút này để thử thời gian xóa dòng");
        testDeleteHold.setTextColor(accent);
        holdCard.addView(testDeleteHold,new LinearLayout.LayoutParams(-1,dp(48)));
        box.addView(holdCard);

        final int[] draftDeleteHold={300+deleteHoldSeek.getProgress()*50};
        java.util.function.IntConsumer updateHoldUi=ms->{
            ms=Math.max(300,Math.min(1200,ms));
            draftDeleteHold[0]=ms;
            holdValue.setText(String.format(Locale.getDefault(),"%.2f giây",ms/1000f).replace(".00","").replace(".50",".5"));
        };
        updateHoldUi.accept(draftDeleteHold[0]);

        deleteHoldSeek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){
            @Override public void onProgressChanged(SeekBar s,int progress,boolean fromUser){
                updateHoldUi.accept(300+progress*50);
            }
            @Override public void onStartTrackingTouch(SeekBar s){}
            @Override public void onStopTrackingTouch(SeekBar s){
                // Áp dụng ngay để người dùng có thể thử mà không cần đóng Cài đặt.
        ...
        View.OnClickListener presetHold=v->{
            int ms=v==hold300?300:(v==hold500?500:(v==hold800?800:1200));
            deleteHoldSeek.setProgress((ms-300)/50);
            updateHoldUi.accept(ms);
            deleteRowHoldMs=ms;
            getSharedPreferences(PREFS,MODE_PRIVATE).edit()
                    .putInt(DELETE_HOLD_KEY,deleteRowHoldMs).apply();
            hapticStep(v);
        ...
        hold800.setOnClickListener(presetHold);
        hold1200.setOnClickListener(presetHold);

        final Handler testHoldHandler=new Handler(Looper.getMainLooper());
        final boolean[] testHoldArmed={false};
        final Runnable testHoldFire=()->{
            if(!testHoldArmed[0])return;
            testHoldArmed[0]=false;
            hapticDanger(testDeleteHold);
            testDeleteHold.setText("✓ Đã đủ thời gian • "+String.format(Locale.getDefault(),"%.2fs",draftDeleteHold[0]/1000f));
            testDeleteHold.postDelayed(()->testDeleteHold.setText("Giữ nút này để thử thời gian xóa dòng"),1100);
        };
        testDeleteHold.setOnTouchListener((v,e)->{
            int a=e.getActionMasked();
            if(a==MotionEvent.ACTION_DOWN){
                testHoldArmed[0]=true;
                testHoldHandler.removeCallbacks(testHoldFire);
                testHoldHandler.postDelayed(testHoldFire,draftDeleteHold[0]);
                return true;
            }
        ...
        AlertDialog settingsDialog=new AlertDialog.Builder(this)
            ...
            .setPositiveButton("Lưu",(d,w)->{
                ...
                deleteRowHoldMs=draftDeleteHold[0];
                saveUiState();renderAll();hapticConfirm(box);
                Toast.makeText(this,"Đã lưu cài đặt • Giữ xóa dòng "+deleteRowHoldMs+" ms",Toast.LENGTH_SHORT).show();
            })
            .setNegativeButton("Đóng",null)
            .create();

        settingsDialog.setOnShowListener(x->{
            Window w=settingsDialog.getWindow();
            if(w!=null){
                int screenH=getResources().getDisplayMetrics().heightPixels;
                int maxH=(int)(screenH*(compact?0.88f:0.82f));
                w.setLayout(compact?WindowManager.LayoutParams.MATCH_PARENT:
    ...
    @Override public boolean dispatchKeyEvent(KeyEvent e){
        ...
        if(e.isCtrlPressed()&&e.isShiftPressed()&&e.getKeyCode()==KeyEvent.KEYCODE_Z){redoLast();return true;}
        if(e.isCtrlPressed()&&e.getKeyCode()==KeyEvent.KEYCODE_Y){redoLast();return true;}
    ...
    void showShareChooser(){
        TableModel t=selected();if(t==null)return;
        GroupModel currentGroup=findGroup(t.groupId);
        ...
        new AlertDialog.Builder(this)
            .setTitle("Chia sẻ / Xuất nhóm")
            .setItems(opts,(d,i)->{
                if(i==0)shareCurrent();
                else if(i==1)shareGroup(g);
                else if(i==2)shareGroupExcel(g);
                else shareGroupWord(g);
            })
            .setNegativeButton("Hủy",null)
            .show();
    }

    String shareTime(long ms){
        if(ms<=0)return "Không rõ";
        return new SimpleDateFormat("dd/MM/yyyy HH:mm",Locale.getDefault()).format(new Date(ms));
    }

    long tableCreatedTime(TableModel t){return t==null?0:t.created;}
    long tableUpdatedTime(TableModel t){return t==null?0:t.updated;}

    String safeFileName(String s){
        if(s==null||s.trim().isEmpty())return "nhom";
        s=s.trim().replaceAll("[\\\\/:*?\"<>|]+","_").replaceAll("\\s+","_");
        return s.length()>48?s.substring(0,48):s;
    }

    void shareCachedFile(File f,String mime,String chooserTitle){
        try{
            ...
            Toast.makeText(this,"Không chia sẻ được file",Toast.LENGTH_LONG).show();
        }
    }
    ...
    void showGroupExportChooser(GroupModel g){
        if(g==null)return;
        String[] opts={"🖼 Ảnh PNG","📊 Excel (.xlsx)","📝 Word (.docx)"};
        new AlertDialog.Builder(this).setTitle("Xuất nhóm "+g.name).setItems(opts,(d,i)->{
            if(i==0)shareGroup(g);
            else if(i==1)shareGroupExcel(g);
            else shareGroupWord(g);
        }).setNegativeButton("Hủy",null).show();
    }
    ...
    void zipText(ZipOutputStream z,String path,String content)throws Exception{
        ZipEntry e=new ZipEntry(path);z.putNextEntry(e);
        z.write(content.getBytes("UTF-8"));z.closeEntry();
    }

    String xlsxCell(String ref,String value,boolean bold){
        String style=bold?" s=\"1\"":"";
        return "<c r=\""+ref+"\" t=\"inlineStr\""+style+"><is><t>"+xmlEsc(value)+"</t></is></c>";
    }

    String xlsxNum(String ref,double value,boolean bold){
        String style=bold?" s=\"1\"":"";
        String n=(Math.rint(value)==value)?String.valueOf((long)value):String.valueOf(value);
        return "<c r=\""+ref+"\""+style+"><v>"+n+"</v></c>";
    }

    void shareGroupExcel(GroupModel g){
        if(g==null)return;
        try{
            File dir=new File(getCacheDir(),"share");dir.mkdirs();
            File f=new File(dir,safeFileName(g.name)+"_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.getDefault()).format(new Date())+".xlsx");
            long shareNow=System.currentTimeMillis();
            ...
            sheet.append("<row r=\"").append(r).append("\">").append(xlsxCell("A"+r,"Ngày sửa nhóm",true)).append(xlsxCell("B"+r,shareTime(groupLatestEditedTime(g)),false)).append("</row>");r++;
            sheet.append("<row r=\"").append(r).append("\">").append(xlsxCell("A"+r,"Ngày xuất/chia sẻ",true)).append(xlsxCell("B"+r,shareTime(shareNow),false)).append("</row>");r++;
            sheet.append("<row r=\"").append(r).append("\">").append(xlsxCell("A"+r,"Tổng nhóm",true)).append(xlsxNum("B"+r,groupTotal(g.id),true)).append("</row>");r+=2;

            int idx=1;
            for(TableModel t:inGroup(g.id)){
                sheet.append("<row r=\"").append(r).append("\">")
                     .append(xlsxCell("A"+r,idx+". "+t.title,true))
                     .append(xlsxCell("B"+r,"Tạo "+shareTime(t.created),false))
                     .append(xlsxCell("C"+r,"Sửa "+shareTime(t.updated),false))
                     .append("</row>");r++;
                if("cancel".equals(t.type)){
                    sheet.append("<row r=\"").append(r).append("\">")
                         .append(xlsxCell("A"+r,"STT",true)).append(xlsxCell("B"+r,"Tên đại lý",true))
                         .append(xlsxCell("C"+r,"Số lượng",true)).append("</row>");r++;
                    int stt=1;
                    for(CancelRow x:t.cancelRows){
                        if(shareHideBlank&&x.blank())continue;
                        sheet.append("<row r=\"").append(r).append("\">")
                             .append(xlsxNum("A"+r,stt++,false)).append(xlsxCell("B"+r,x.agent,false))
                             .append(xlsxNum("C"+r,x.qty,false)).append("</row>");r++;
                    }
                }else{
                    sheet.append("<row r=\"").append(r).append("\">")
                         .append(xlsxCell("A"+r,"STT",true)).append(xlsxCell("B"+r,"Đơn giá",true))
                         .append(xlsxCell("C"+r,"SL",true)).append(xlsxCell("D"+r,"Thành tiền",true)).append("</row>");r++;
                    int stt=1;
                    for(CalcRow x:t.calcRows){
                        if(shareHideBlank&&x.blank())continue;
                        sheet.append("<row r=\"").append(r).append("\">")
                             .append(xlsxNum("A"+r,stt++,false)).append(xlsxNum("B"+r,x.price,false))
                             .append(xlsxNum("C"+r,x.qty,false)).append(xlsxNum("D"+r,x.price*x.qty,false)).append("</row>");r++;
                    }
                }
    ...
    String wordP(String text,boolean bold){
        return "<w:p><w:r>"+(bold?"<w:rPr><w:b/></w:rPr>":"")+"<w:t xml:space=\"preserve\">"+xmlEsc(text)+"</w:t></w:r></w:p>";
    }

    String wordCell(String text,boolean bold){
        return "<w:tc><w:tcPr><w:tcW w:w=\"2400\" w:type=\"dxa\"/></w:tcPr>"+wordP(text,bold)+"</w:tc>";
    }

    void shareGroupWord(GroupModel g){
        if(g==null)return;
        try{
            File dir=new File(getCacheDir(),"share");dir.mkdirs();
            File f=new File(dir,safeFileName(g.name)+"_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.getDefault()).format(new Date())+".docx");
            long shareNow=System.currentTimeMillis();
            ...
            doc.append(wordP(g.name,true));
            doc.append(wordP("Ngày tạo: "+shareTime(groupCreatedTime(g)),false));
            doc.append(wordP("Ngày sửa: "+shareTime(groupLatestEditedTime(g)),false));
            doc.append(wordP("Ngày chia sẻ: "+shareTime(shareNow),false));
            doc.append(wordP("Tổng nhóm: "+fmt(groupTotal(g.id)),true));

            int idx=1;
            for(TableModel t:inGroup(g.id)){
                doc.append(wordP(idx+". "+t.title,true));
                doc.append(wordP("Tạo "+shareTime(t.created)+" • Sửa "+shareTime(t.updated),false));
                doc.append("<w:tbl><w:tblPr><w:tblBorders><w:top w:val=\"single\" w:sz=\"4\"/><w:left w:val=\"single\" w:sz=\"4\"/><w:bottom w:val=\"single\" w:sz=\"4\"/><w:right w:val=\"single\" w:sz=\"4\"/><w:insideH w:val=\"single\" w:sz=\"4\"/><w:insideV w:val=\"single\" w:sz=\"4\"/></w:tblBorders></w:tblPr>");
                if("cancel".equals(t.type)){
                    doc.append("<w:tr>").append(wordCell("STT",true)).append(wordCell("Tên đại lý",true)).append(wordCell("Số lượng",true)).append("</w:tr>");
                    int stt=1;
                    for(CancelRow x:t.cancelRows){
                        if(shareHideBlank&&x.blank())continue;
                        doc.append("<w:tr>").append(wordCell(String.valueOf(stt++),false)).append(wordCell(x.agent,false)).append(wordCell(fmt(x.qty),false)).append("</w:tr>");
                    }
                }else{
                    doc.append("<w:tr>").append(wordCell("STT",true)).append(wordCell("Đơn giá",true)).append(wordCell("SL",true)).append(wordCell("Thành tiền",true)).append("</w:tr>");
                    int stt=1;
                    for(CalcRow x:t.calcRows){
                        if(shareHideBlank&&x.blank())continue;
                        doc.append("<w:tr>").append(wordCell(String.valueOf(stt++),false)).append(wordCell(fmt(x.price),false)).append(wordCell(fmt(x.qty),false)).append(wordCell(fmt(x.price*x.qty),false)).append("</w:tr>");
                    }
                }
                doc.append("</w:tbl>");
                doc.append(wordP("Tổng bảng: "+fmt(t.total()),true));
                idx++;
            }
            doc.append("<w:sectPr><w:pgSz w:w=\"11906\" w:h=\"16838\"/><w:pgMar w:top=\"720\" w:right=\"720\" w:bottom=\"720\" w:left=\"720\"/></w:sectPr></w:body></w:document>");

            try(ZipOutputStream z=new ZipOutputStream(new FileOutputStream(f))){
                zipText(z,"[Content_Types].xml","<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><Types xmlns=\"http://schemas.openxmlformats.org/package/2006/content-types\"><Default Extension=\"rels\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/><Default Extension=\"xml\" ContentType=\"application/xml\"/><Override PartName=\"/word/document.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml\"/></Types>");
                zipText(z,"_rels/.rels","<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument\" Target=\"word/document.xml\"/></Relationships>");
                zipText(z,"word/document.xml",doc.toString());
            }
            shareCachedFile(f,"application/vnd.openxmlformats-officedocument.wordprocessingml.document","Chia sẻ Word nhóm");
        }catch(Exception e){
            Toast.makeText(this,"Không tạo được file Word: "+e.getMessage(),Toast.LENGTH_LONG).show();
        }
    }
    ...
    void shareViewAsPng(View report,String prefix,String chooserTitle){
        try{
            ...
            if(height>dp(12000)){
                shareViewAsPdf(report,prefix,chooserTitle.replace("ảnh","PDF"));
                return;
            }
    ...
    void shareViewAsPdf(View report,String prefix,String chooserTitle){
        try{
            int width=Math.min(dp(900),Math.max(dp(560),getResources().getDisplayMetrics().widthPixels));
            ...
            report.layout(0,0,width,report.getMeasuredHeight());
            int pageH=dp(1200),fullH=Math.max(1,report.getMeasuredHeight());
            android.graphics.pdf.PdfDocument pdf=new android.graphics.pdf.PdfDocument();
            int pageNo=1;
            for(int y=0;y<fullH;y+=pageH){
                android.graphics.pdf.PdfDocument.PageInfo pi=new android.graphics.pdf.PdfDocument.PageInfo.Builder(width,pageH,pageNo++).create();
                android.graphics.pdf.PdfDocument.Page page=pdf.startPage(pi);
                Canvas c=page.getCanvas();c.drawColor(Color.WHITE);c.save();c.translate(0,-y);report.draw(c);c.restore();pdf.finishPage(page);
            }
            File dir=new File(getCacheDir(),"share");dir.mkdirs();
            File f=new File(dir,prefix+"-"+System.currentTimeMillis()+".pdf");
            try(FileOutputStream os=new FileOutputStream(f)){pdf.writeTo(os);}pdf.close();
            Uri uri=Uri.parse("content://com.vinh.listcalculatorfold2.share/"+Uri.encode(f.getName()));
            Intent send=new Intent(Intent.ACTION_SEND);send.setType("application/pdf");
            send.putExtra(Intent.EXTRA_STREAM,uri);send.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            startActivity(Intent.createChooser(send,chooserTitle));
        }catch(Exception e){Toast.makeText(this,"Không tạo được PDF",Toast.LENGTH_LONG).show();}
    }
    ...
    LinearLayout buildGroupShareView(GroupModel g){
        ...
        long shareNow=System.currentTimeMillis();
        LinearLayout timeBox=new LinearLayout(this);timeBox.setOrientation(LinearLayout.VERTICAL);
        TextView createdLine=text("Ngày tạo: "+shareTime(groupCreatedTime(g)),11,false);
        TextView updatedLine=text("Ngày sửa: "+shareTime(groupLatestEditedTime(g)),11,false);
        TextView sharedLine=text("Ngày chia sẻ: "+shareTime(shareNow),11,true);
        createdLine.setTextColor(Color.GRAY);updatedLine.setTextColor(Color.GRAY);sharedLine.setTextColor(muted);
        timeBox.addView(createdLine);timeBox.addView(updatedLine);timeBox.addView(sharedLine);
    ...
    LinearLayout buildShareView(TableModel t){
        ...
        long shareNow=System.currentTimeMillis();
        TextView createdLine=text("Ngày tạo: "+shareTime(tableCreatedTime(t)),11,false);createdLine.setTextColor(Color.GRAY);r.addView(createdLine);
        TextView updatedLine=text("Ngày sửa: "+shareTime(tableUpdatedTime(t)),11,false);updatedLine.setTextColor(Color.GRAY);r.addView(updatedLine);
        TextView sharedLine=text("Ngày chia sẻ: "+shareTime(shareNow),11,true);sharedLine.setTextColor(muted);r.addView(sharedLine);
    ...
    void save(){
        ...
        saveScheduled=true;
        saveHandler.postAtTime(()->saveNow(),"SAVE",SystemClock.uptimeMillis()+350);
    }
    void saveNow(){try{JSONObject root=new JSONObject();JSONArray ga=new JSONArray();for(GroupModel g:groups)ga.put(g.json());JSONArray ta=new JSONArray();for(TableModel t:tables)ta.put(t.json());JSONArray tga=new JSONArray();for(TotalGroupModel g:totalGroups)tga.put(g.json());root.put("groups",ga).put("tables",ta).put("totalGroups",tga);getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(DATA,root.toString()).apply();saveUiState();saveScheduled=false;}catch(Exception ignored){saveScheduled=false;}}
    void flushSave(){
    ...
    void load(){String s=getSharedPreferences(PREFS,MODE_PRIVATE).getString(DATA,null);if(s!=null){try{JSONObject o=new JSONObject(s);JSONArray ga=o.optJSONArray("groups");if(ga!=null)for(int i=0;i<ga.length();i++)groups.add(GroupModel.from(ga.getJSONObject(i)));JSONArray ta=o.optJSONArray("tables");if(ta!=null)for(int i=0;i<ta.length();i++)tables.add(TableModel.from(ta.getJSONObject(i)));JSONArray tga=o.optJSONArray("totalGroups");if(tga!=null)for(int i=0;i<tga.length();i++)totalGroups.add(TotalGroupModel.from(tga.getJSONObject(i)));return;}catch(Exception ignored){}}migrateOld();}
    ...
    static class TotalGroupModel{
        String id,name;
        final HashSet<String> tableIds=new HashSet<>();
        final HashSet<String> groupIds=new HashSet<>();
        long created=System.currentTimeMillis();

        JSONObject json()throws Exception{
            JSONObject o=new JSONObject().put("id",id).put("name",name).put("created",created);
            JSONArray ta=new JSONArray();for(String s:tableIds)ta.put(s);
            JSONArray ga=new JSONArray();for(String s:groupIds)ga.put(s);
            o.put("tableIds",ta).put("groupIds",ga);
            return o;
        }

        static TotalGroupModel from(JSONObject o){
            TotalGroupModel g=new TotalGroupModel();
            g.id=o.optString("id");
            g.name=o.optString("name","Nhóm cộng tổng");
            g.created=o.optLong("created",System.currentTimeMillis());
            JSONArray ta=o.optJSONArray("tableIds");if(ta!=null)for(int i=0;i<ta.length();i++)g.tableIds.add(ta.optString(i));
            JSONArray ga=o.optJSONArray("groupIds");if(ga!=null)for(int i=0;i<ga.length();i++)g.groupIds.add(ga.optString(i));
            return g;
        }
    }
    ...
    static class GroupModel{
        String id,name;
        boolean pinned=false,locked=false;
        long created=System.currentTimeMillis();
        JSONObject json()throws Exception{
            return new JSONObject().put("id",id).put("name",name).put("pinned",pinned).put("locked",locked).put("created",created);
        }
        static GroupModel from(JSONObject o){
            GroupModel g=new GroupModel();
            g.id=o.optString("id");g.name=o.optString("name","Nhóm");g.pinned=o.optBoolean("pinned",false);g.locked=o.optBoolean("locked",false);
    ...
    void applyButtonStyle(Button b,int bg,int fg,int stroke,int radius){
        ...
        b.setTextColor(fg);
        GradientDrawable d=new GradientDrawable();
        d.setColor(bg);d.setStroke(dp(1),stroke);d.setCornerRadius(dp(radius));
        b.setBackground(d);b.setStateListAnimator(null);
    }
